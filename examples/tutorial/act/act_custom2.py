...
def main() -> None:
    ...
    print("\n[조작]  <- -> : 회전  |  S : 레코딩  |  E : 레코딩 종료  |  R : 리플레이  |  T : 리플레이 중단  |  ESC/Q : 종료\n")
    ...
    while True:
        ...
        if key in (27, ord('q'), ord('Q')):
            ...
            if not is_replaying and not is_recording:
                ...
                if os.path.isdir(replay_data_dir):
                    # finalize 후 다시 로드 (현재 세션의 미저장 메타데이터 플러시)
                    dataset.finalize()
        ...
        if is_replaying:
            ...
            status = f"  [REC {frame_count}]"
        else:
            status = ""
        label(world_bgr, f"World View  |  Pan: {deg:+.1f} deg{status}")
        label(cam_bgr,   "Camera View  |  [<- ->] [S]Rec [E]Stop [R]Replay [T]Stop [Q]Quit")
