...
def main():
    ...
    if pretrained_path.exists():
        ...
        pretrained_policy = ACTPolicy.from_pretrained(output_directory)
    ...
    while not done:
        for batch in dataloader:
            ...
            if step % 10 == 0:
                with torch.no_grad():
                    # 예측 로그는 inference 경로를 사용하므로 잠깐 eval 모드로 전환한다.
                    policy.eval()
                    pred = policy.select_action(batch)  # (B, action_dim)
                    policy.train()
                target_vals = batch["action"][:, 0, :]   # (B, action_dim)
                print(f"  step {step:>5d}/{training_steps}  |  loss: {loss_val:.4f}"
                      f"  |  target: {target_vals[0].tolist()}  pred: {pred[0].tolist()}")
